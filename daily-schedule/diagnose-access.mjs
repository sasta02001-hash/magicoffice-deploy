...
for(const [label,route] of [['project','/v9/projects/'+project],['production-alias','/v4/aliases/magicoffice-data.vercel.app'],['deployment-id','/v13/deployments/'+request.expectedDeploymentId],['deployment-host','/v13/deployments/magicoffice-data.vercel.app']]) {
 ...
 if(!r.ok) failures++;
 if(!process.argv.includes('--diagnostic')) break;
}
if(failures) throw new Error('VERCEL_CREDENTIAL_ACCESS_DENIED: The configured Actions VERCEL_TOKEN cannot read the authorized data project. Account owner must restore that credential access; no deployment was attempted.');
