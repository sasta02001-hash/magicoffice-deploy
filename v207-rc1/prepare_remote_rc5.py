...
REVISION = "rc5-release-candidate"
ASSET_COMMIT = "d0c26168ccb347a858f7339aa271687f6d60e9db"
...
def inline_stylesheets(html: str) -> tuple[str, int]:
    ...
    for match in list(re.finditer(r"<link\b[^>]*>", html, flags=re.I)):
        ...
        html = html.replace(tag, f'<style data-inlined-from="{href}">\n{css}\n</style>', 1)
        count += 1
    return html, count
...
def main() -> None:
    ...
    html = html.replace("href='/site.webmanifest'", f"href='{CDN_ROOT}site.webmanifest'")
    html = re.sub(r'data-remote-preview="[^"]*"', f'data-remote-preview="{REVISION}"', html)
    if f'data-remote-preview="{REVISION}"' not in html:
        html = html.replace("<html ", f'<html data-remote-preview="{REVISION}" ', 1)
    ...
    report = {
        "version": VERSION,
        "remotePreviewRevision": REVISION,
    ...
    print("MAGICOFFICE_V207_REMOTE_RC5", json.dumps(report, ensure_ascii=False))
