...
report={'attempted':True,'url':ORIGIN,'environment':'GitHub Actions Ubuntu; actual deployed network pages; Chromium and Linux WebKit engines. Not physical Android, iPhone, macOS, LINE or Instagram.','cases':[],'failures':[]}
s=requests.Session();r=s.get(URL,timeout=50);r.raise_for_status()
if RELEASE not in r.text:
 r=s.get(ORIGIN+'/',timeout=50);r.raise_for_status()
...
with sync_playwright() as p:
 for engine in ['chromium','webkit']:
  ...
  for width,height in ([(1440,900),(320,568),(390,844),(430,932)] if engine=='chromium' else [(1440,900),(390,844)]):
   ...
   ctx.add_cookies(cookies);page=ctx.new_page();page.set_default_timeout(12000)
   errors=[];bad=[]
   ...
   try:
    ...
    try:
     ...
     page.locator('[data-video-sound]').click();checks['videoMute']=page.locator('video').evaluate('(v)=>v.muted')
     page.locator('[data-video-sound]').click();checks['videoUnmute']=not page.locator('video').evaluate('(v)=>v.muted')
    ...
    for index in range(16):
     page.locator('[data-profile-id]').nth(index).click()
     ...
     profile_checks.append(x['grid']=='grid' and x['overflow']<=1 and len(x['fields'])>=3 and x['photo']>=600)
     if index==0:page.screenshot(path=str(OUT/f'{label}-profile.png'))
     page.locator('#profile-dialog [data-dialog-close]').click()
    checks['allProfilesFormatted']=all(profile_checks)
    ...
    if width<961:
     ...
     page.locator('#purchase-dialog [data-dialog-close]').click()
