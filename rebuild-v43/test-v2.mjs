...
async function run(name, viewport, mobile = false) {
  ...
  if (mobile) {
    await page.setUserAgent('Mozilla/5.0 (iPhone; CPU iPhone OS 18_7 like Mac OS X) AppleWebKit/605.1.15 Version/18.7 Mobile/15E148 Safari/604.1');
  }
  ...
  page.on('response', (response) => { if (response.status() >= 400 && !response.url().includes('/api/')) badResponses.push({ url: response.url(), status: response.status() }); });
  ...
  const firstCategory = await page.$('#menu-panel [data-menu-category]:not([data-menu-category="ALL"])');
  if (firstCategory) {
    const categoryId = await firstCategory.evaluate((node) => node.dataset.menuCategory);
    await firstCategory.click();
    await page.waitForFunction((value) => document.querySelector(`[data-menu-category="${value}"]`)?.classList.contains('active'), {}, categoryId);
  ...
  const state = await page.evaluate(() => {
    ...
    return {
      ...
      titleWhiteSpace: computed?.whiteSpace,
      ...
      eventDetailsCount: document.querySelectorAll('#event-details details').length,
      ...
      bookingLinks: [...document.querySelectorAll('a')].filter((a) => a.textContent.includes('立即訂位')).map((a) => a.href),
    };
  });
  ...
  report.cases[name] = { state, pageErrors, consoleErrors, failedRequests, badResponses };
  ...
  if (!(state.videoCurrentTime > 0.2) || state.videoPaused) report.failures.push(`${name}: video playback`);
  ...
  if (state.eventCount !== 5 || state.eventDetailCount !== 5 || state.eventDetailsCount < 13) report.failures.push(`${name}: events ${state.eventCount}/${state.eventDetailCount}/${state.eventDetailsCount}`);
  ...
  if (!state.bookingLinks.length) report.failures.push(`${name}: booking link missing`);
